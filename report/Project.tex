...
% ===================== REPLACED M3 (UNION OF BOTH VERSIONS) =====================

\subsection{Methodology 3: Mean-Variance Optimal Portfolio Construction (M3)}
\label{sec:methodology-3}

\subsection{Introduction and Key Motivation}
This methodology implements a Markowitz mean-variance optimization framework to construct delta-neutral hedge portfolios for short strangle positions. The key motivation is to move beyond simple delta hedging by incorporating the covariance structure between hedge assets, thereby achieving superior risk reduction while maintaining delta neutrality. \\

\noindent The methodology addresses three critical objectives:
\begin{itemize}
    \item \textbf{Delta Neutrality}: Ensure the hedge portfolio exactly offsets the strangle's delta exposure
    \item \textbf{Variance Minimization}: Optimize portfolio weights to minimize overall variance using the covariance structure
    \item \textbf{Risk-Return Tradeoff}: Incorporate expected returns for mean-variance optimization when desired
\end{itemize}

\noindent This approach is grounded in the Mean-Variance Analysis and Capital Asset Pricing Model (CAPM) framework from Lecture 4 and 5 and represents a significant advancement over naive 1:1 delta hedging by accounting for the complex risk relationships between Bitcoin spot, futures, and cash positions.\\

\noindent It extends these classical results to incorporate derivative hedging constraints, specifically delta neutrality for options portfolios. The implementation demonstrates how portfolio optimization theory can be practically applied to complex hedging problems in financial engineering. \\

\noindent The approach shows that optimal hedging requires considering not just the direction of exposure (delta) but also the covariance structure between hedging instruments, representing a more sophisticated approach to risk management than traditional Greek-based hedging.

\subsection{Technical Implementation}

\subsubsection{Optimization Problem Formulation}
The core optimization problem follows the Markowitz mean-variance framework:

\begin{align*}
\text{minimize} \quad & \mathbf{w}^\top \boldsymbol{\Sigma}\, \mathbf{w} \\
\text{subject to} \quad & \sum w_i = 1 \quad \text{(budget constraint)} \\
& \boldsymbol{\delta}^\top \mathbf{w} = -\delta_{\text{net}} \quad \text{(delta neutrality)} \\
& w_i \geq 0 \quad \text{(long-only constraint, optional)}
\end{align*}

\noindent In implementation, we also consider a regularized convex objective and practical constraints:
\begin{align}
...
where:
\begin{itemize}
    \item $\mathbf{w} = [w_{\text{spot}}, w_{\text{futures}}, w_{\text{cash}}]^\top$ are portfolio weights
    \item $\boldsymbol{\Sigma}$ (or $\Sigma$) is the covariance matrix from Methodology 2 (EWMA)
    \item $\boldsymbol{\delta} = [1.0, 1.0, 0.0]^\top$ are asset deltas
    \item $\delta_{\text{net}}$ (or $\delta_{\text{strangle}}$) is the net delta from Methodology 1
    \item $\lambda_{\text{reg}}$ is a regularization term to penalize extreme positions
\end{itemize}

\subsubsection{Key Technical Components}

\noindent \textbf{\normalsize Asset Universe and Constraints}
\vspace{0.5em}

\noindent The optimization considers three hedge assets:
\begin{itemize}
    \item \textbf{BTC Spot} ($\delta = 1.0$): Direct Bitcoin exposure
    \item \textbf{BTC Futures} ($\delta = 1.0$): Leveraged Bitcoin exposure with different risk characteristics
    \item \textbf{USDT Cash} ($\delta = 0.0$): Risk-free asset earning 5\% annual risk-free rate
\end{itemize}

\noindent The delta-neutrality constraint ensures that the hedge portfolio's net delta exactly offsets the short strangle's delta exposure, making the combined position delta-neutral. \\

\noindent \textbf{\normalsize Numerical Robustness}
\vspace{0.5em}

\noindent Several techniques ensure numerical stability:
\begin{itemize}
    \item \textbf{Positive Definite Enforcement}: Eigenvalue flooring for covariance matrices
    \item \textbf{Fallback Mechanisms}: Smart fallback strategies when optimization fails
    \item \textbf{Regularization}: $L_2$ regularization to prevent extreme weights
\end{itemize}

\noindent \textbf{\normalsize Advanced Constraints}
\vspace{0.5em}

\noindent The implementation includes sophisticated constraints for practical deployment:
\begin{itemize}
    \item Concentration limits (maximum 33\% in single asset)
    \item Liquidity requirements (minimum 10\% cash allocation)
    \item Leverage constraints ($-0.5 \leq w_i \leq 1.5$)
    \item Target Sharpe ratio constraints for risk budgeting
\end{itemize}

\subsubsection{Expected Returns Specification}
Expected returns are computed using the CAPM framework:
\[
\mathbb{E}[r_i] = r_f + \beta_i (\mathbb{E}[r_m] - r_f)
\]
with default parameters: $\beta_{\text{spot}} = 1.5$, $\beta_{\text{futures}} = 1.5$, $\mathbb{E}[r_m] = 10\%$, $r_f = 5\%$.

\subsubsection{Efficient Frontier Analysis}
The methodology includes computation of the delta-constrained efficient frontier, showing the optimal risk-return tradeoff available while maintaining delta neutrality. This frontier is necessarily inside the unconstrained efficient frontier, quantifying the cost of the hedging constraint.

\subsection{Implementation Features}

\subsubsection{Optimization Framework}
The implementation uses \texttt{cvxpy} with the OSQP solver for convex optimization. Key features include:

\begin{itemize}
    \item \textbf{Multiple Objective Functions}: Pure minimum variance or mean-variance utility
    \item \textbf{Risk Aversion Parameterization}: Flexible risk-return tradeoff specification
    \item \textbf{Comprehensive Diagnostics}: Portfolio metrics and optimization status tracking
\end{itemize}

\subsubsection{Hedge Effectiveness Measurement}
The methodology implements hedge effectiveness quantification:
\[
\text{Effectiveness} = 1 - \frac{\sigma^2_{\text{hedged}}}{\sigma^2_{\text{unhedged}}}
\]
providing a clear metric for comparing MV optimal vs. naive hedging performance.

\subsubsection{Comparison Baseline}
A naive delta hedge baseline is provided for comparison:
\[
\mathbf{w}_{\text{naive}} = [0, -\delta_{\text{strangle}}, 1 + \delta_{\text{strangle}}]
\]
This simple 1:1 futures hedge ignores covariance structure and serves as a performance benchmark.
...
This project utilized AI tools (including ChatGPT) as a supporting assistant during development and report preparation. Specifically, AI assistance was used for (i) debugging and improving code reliability (e.g., identifying implementation bugs, suggesting numerical stability practices, and helping interpret error messages from Python libraries such as \texttt{cvxpy}); (ii) polishing academic writing for clarity, grammar, and structure while preserving the authors' intended meaning; and (iii) high-level research support such as brainstorming related concepts (e.g., mean-variance optimization, CAPM-based expected return specification, and risk measurement terminology) and helping organize the report into a professional academic format. All quantitative modeling choices, experimental design (train/validation/test splits), implementation decisions, result interpretation, and final report content were reviewed and finalized by the authors. AI outputs were treated as suggestions and were selectively adopted only after verification.
