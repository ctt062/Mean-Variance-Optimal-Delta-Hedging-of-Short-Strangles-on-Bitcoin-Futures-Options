...
# -------------- Poor performance if consider transaction costs -------
# # ============================================================================
# # OPTIMIZE HEDGE PORTFOLIO WITH TRANSACTION COSTS
# # ============================================================================

# def optimize_hedge_portfolio(cov_matrix: np.ndarray,
#                              net_delta: float,
#                              initial_weights: np.ndarray = None,  # Add initial portfolio
#                              tc_rate: float = 0.0005,  # 0.05% Deribit taker fee
#                              concentration_limit: float = 0.5,  # max 50% in single asset
#                              target_sharpe: float = None,  # target sharpe ratio
#                              expected_returns: np.ndarray = None,
#                              risk_aversion: float = 0.0) -> Tuple[np.ndarray, Dict]:
#     """
#     Optimize hedge portfolio for short strangle (Methodology 3) with transaction costs.
...
#         Current portfolio weights [spot, futures, cash]
#         If None, assumes starting from cash-only portfolio
#     tc_rate : float
#         Transaction cost rate (e.g., 0.0005 for 0.05%)
#     expected_returns : np.ndarray, optional
#         Expected returns for mean-variance optimization
#     risk_aversion : float
#         Risk aversion parameter (0 = pure min variance)
...
#     4. (Optionally) maximize expected return given variance
#     """
#     n_assets = 3  # spot, futures, cash
...
#     # Default initial weights (all in cash)
#     if initial_weights is None:
#         initial_weights = np.array([0.0, 0.0, 1.0])
    
#     # Decision variables
#     w = cp.Variable(n_assets)  # Target weights
#     w_buy = cp.Variable(n_assets, nonneg=True)  # Buy amounts
#     w_sell = cp.Variable(n_assets, nonneg=True)  # Sell amounts
    
#     # Transaction cost calculation
#     # We need to buy or sell to get from initial_weights to w
#     # w = initial_weights + w_buy - w_sell
#     # where w_buy >= 0, w_sell >= 0, and w_buy * w_sell = 0 (complementarity)
    
#     # Portfolio rebalancing constraint
#     rebalance_constraint = w == initial_weights + w_buy - w_sell
    
#     # Transaction cost (proportional to turnover)
#     transaction_cost = tc_rate * cp.sum(w_buy + w_sell)
    
#     # Net transaction cost (as a negative return)
#     net_expected_return = expected_returns @ w - transaction_cost
    
#     # Portfolio variance
#     portfolio_variance = cp.quad_form(w, cov_matrix)
    
#     # Objective: min variance - λ * expected return (with transaction cost)
#     if risk_aversion > 0:
#         # Mean-variance utility with transaction costs
#         objective = cp.Minimize(risk_aversion/2 * portfolio_variance - net_expected_return)
#     else:
#         # Pure minimum variance with transaction cost penalty
#         # Add small penalty for transaction costs even in min variance
#         transaction_penalty = 0.1  # Weight for transaction cost penalty
#         objective = cp.Minimize(portfolio_variance + transaction_penalty * transaction_cost)
    
#     # Constraints
#     constraints = [
#         cp.sum(w) == 1,  # Fully invested
#         rebalance_constraint,
#         w_buy >= 0,
#         w_sell >= 0,
#         w >= -0.5,  # Allow small shorts for flexibility
#         w <= 1.5,  # Cap leverage
...
#     # Complementarity constraint: can't buy and sell the same asset simultaneously
#     for i in range(n_assets):
#         constraints.append(w_buy[i] * w_sell[i] == 0)
    
#     # Maximum risk constraint (optional)
#     if target_sharpe is not None and expected_returns is not None:
#         # Adjust target sharpe for transaction costs
#         adjusted_returns = expected_returns - tc_rate
#         max_volatility = adjusted_returns / target_sharpe
#         max_var = max_volatility**2
#         constraints.append(portfolio_variance <= max_var)
...
#         if problem.status == 'optimal':
#             weights = w.value
#             buy_amounts = w_buy.value
#             sell_amounts = w_sell.value
            
#             # Calculate transaction costs
#             total_turnover = np.sum(buy_amounts) + np.sum(sell_amounts)
#             total_tc = tc_rate * total_turnover
...
#                 'expected_return': float(expected_returns @ weights),
#                 'net_expected_return': float(net_expected_return.value),
#                 'sharpe': float((expected_returns @ weights - RISK_FREE_RATE_DAILY) / 
#                               (np.sqrt(portfolio_variance.value) + 1e-10)),
#                 'net_sharpe': float((net_expected_return.value - RISK_FREE_RATE_DAILY) / 
#                                    (np.sqrt(portfolio_variance.value) + 1e-10)),
#                 'transaction_cost': float(total_tc),
#                 'turnover': float(total_turnover),
#                 'buy_amounts': buy_amounts.tolist(),
#                 'sell_amounts': sell_amounts.tolist()
#             }
            
#             return weights, diagnostics
        
#         else:
#             # Fallback: simple hedge with transaction cost consideration
#             # Only rebalance if benefit exceeds cost
#             target_weights = np.array([0.0, -net_delta, 1 + net_delta])
#             target_weights = np.clip(target_weights, 0, 1)
            
#             # Calculate turnover needed
#             turnover = np.abs(target_weights - initial_weights).sum()
#             expected_benefit = (expected_returns @ target_weights - 
#                                 expected_returns @ initial_weights)
#             tc_cost = tc_rate * turnover
            
#             # Only rebalance if expected benefit > transaction cost
#             if expected_benefit > tc_cost:
#                 fallback_weights = target_weights
#                 rebalance_flag = True
#             else:
#                 fallback_weights = initial_weights
